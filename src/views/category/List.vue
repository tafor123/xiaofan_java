...
<!--          scope.row就是当前行-->
          <el-button type="success" @click="$router.push('/editCategory?id=' + scope.row.id)">编辑</el-button>
          <el-popconfirm
          ...
          <el-button style="margin-left: 4px" type="primary" @click="handleAdd(scope.row)" v-if="!scope.row.pid" >添加二级分类</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  data(){
    return{
      ...
      rules: {
        name:[
          {required: true, message: '请输入新的图书分类名称', trigger: 'blur'},
